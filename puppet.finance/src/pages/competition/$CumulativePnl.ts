...
export const $CumulativePnl = (config: ICompetitonCumulativeRoi) => component((
  ...
  [sortByChange, sortByChangeTether]: Behavior<ISortBy<IBlueberryLadder>, ISortBy<IBlueberryLadder>>,
  ...
  const sortBy: Stream<ISortBy<IBlueberryLadder>> = mergeArray([
  ...
  const requestCompetitionLeaderboard = map((params) => {
      ...
      $column(layoutSheet.spacing)(
        $row(layoutSheet.spacing, style({ alignItems: 'flex-end' }))(
          ...
          $responsiveFlex(layoutSheet.spacingSmall, style({ placeContent: 'flex-end', alignItems: 'flex-end' }))(
            ...
            }))(map(params => '~' + formatReadableUSD(params.estPrizePool), competitionLeaderboard))
          ),
        ),

        $CardTable({
          dataSource: map(xx => xx.list, competitionLeaderboard),
          sortBy,
          columns: [
            {
              $head: $text('Account'),
