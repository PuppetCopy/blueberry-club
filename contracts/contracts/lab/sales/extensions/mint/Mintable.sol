...
abstract contract Mintable is Payable {
    ...
    function setFinish(uint64 _finish) external onlyOwner {
        _state.finish = _finish;
    }

    function setMaxPerWallet(uint192 _maxMintable) external onlyOwner {
        _state.maxMintable = _maxMintable;
    }

    function _mint(address to, uint120 amount, MintRule memory rule) internal {
        MintState memory state_ = _state;
        uint256 totalMinted_ = totalMinted[msg.sender] + amount;
