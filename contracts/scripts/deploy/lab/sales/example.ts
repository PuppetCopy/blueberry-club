...
export default async function(data: SaleData, GBC: string, LAB: string, TREASURY: string): Promise<SaleExample> {
  ...
  const instance = await factory.deploy(TREASURY, data.item, data.supply, data.max_per_tx, data.cost, data.start, data.whitelist_start, data.cost, data.whitelist_max_mint, GBC, LAB)
